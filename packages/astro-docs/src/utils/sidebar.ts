...
function toBadge(input: SidebarEntry["data"]["sidebar"]["badge"]): Badge | undefined {
...
function renderNode(
  ...
  const linkItems = node.links.map((e) => {
    ...
    return {
      sortKey: { order: orderOf(e), label: e.data.sidebar.label ?? e.data.title },
      resolved: {
        type: "link" as const,
...
function resolveItem(
  ...
  if ("items" in item) {
    const items = item.items
      .map((i) => resolveItem(i, entries, base, currentSlug))
      .flat()
...
function normalizeConfigBadge(badge: unknown): Badge | undefined {
  ...
  const b = badge as { text: string; variant?: Badge["variant"]; class?: string };
  return { text: b.text, variant: b.variant ?? "default", class: b.class };
}

export function buildSidebar(params: BuildSidebarParams): ResolvedSidebarItem[] {
  const { config, entries, base, currentSlug } = params;
  if (!config) {
    return autogenerate(entries, "", base, currentSlug, false);
  }
  return config
    .map((item) => resolveItem(item, entries, base, currentSlug))
    .flat()
    .filter((i): i is ResolvedSidebarItem => i !== null);
}
