...
describe("remarkAsides", () => {
  it("transforms a ::: container directive into a callout", () => {
    ...
    expect(node.children[0].data.hProperties.className).toContain("docs-callout-title");
  ...
  it("uses a directive label as the title", () => {
    const node: any = {
      ...
      name: "note",
      children: [
        { type: "paragraph", data: { directiveLabel: true }, children: [{ type: "text", value: "Heads up" }] },
        para("Body text."),
      ],
    };
    run({ type: "root", children: [node] });
  ...
  it("ignores unknown directive names", () => {
    const node: any = { type: "containerDirective", name: "unknown", children: [para("x")] };
    run({ type: "root", children: [node] });
