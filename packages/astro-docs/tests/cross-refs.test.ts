		...
		children: [
			{
				...
				children: [{ type: "text", value: "The overview." }],
			},
...
const file = {
	data: { astro: { frontmatter: { chapter: 1, numberSections: true } } },
};
	...
	it("leaves unknown references as literal text", () => {
		const tree: any = {
			...
			children: [
				{
					type: "paragraph",
					children: [{ type: "text", value: "See @fig-missing." }],
				},
			],
		};
		remarkCrossReferences()(tree as any, file);
		const text = tree.children[0].children
			.map((c: any) => c.value ?? "")
			.join("");
